...
:root {
  ...
  --bp-color-biblio-surface-soft: #fbfcfe;
  --bp-color-biblio-link: #1c5fb8;
  ...
  --bp-space-5: 1.5rem;
  --bp-space-6: 2rem;
  --bp-duration-fast: 0.12s;
  --bp-duration-base: 0.16s;
  --bp-duration-slow: 0.24s;
  --bp-ease: ease;
  --bp-fs-badge: 0.72rem;
...
@media (prefers-color-scheme: dark) {
  :root {
    ...
    --bp-color-biblio-surface-soft: #18242f;
    --bp-color-biblio-link: #5aa0ff;
...
/* ---- Rendering-tier marker ------------------------------------------------ */
/* A quiet corner glyph on every Lean code block recording which pipeline
   produced it (see `Informal.NodeCard.tierMarker`): re-elaborated from source,
   signature-only re-elaboration, syntactic highlight, or a pretty-print of the
   compiled declaration. The distinction is real — only the first tier has been
   type-checked again — and the fallback between tiers is otherwise silent.

   Colors come from the existing `--bp-color-text-faint` / `--bp-color-accent-*`
   tokens, which carry light and dark values in all four scheme blocks above, so
   the marker themes for free. It is `aria-label`led and title'd rather than
   decorative, and never intercepts text selection in the code. */
.bp_tier_marker {
  position: absolute;
  top: 0.15rem;
  ...
  font-size: var(--bp-fs-badge, 0.7rem);
  line-height: 1;
  opacity: 0.55;
  cursor: help;
  user-select: none;
  transition: opacity var(--bp-duration-fast, 120ms) var(--bp-ease, ease);
}

.bp_tier_marker:hover,
.bp_tier_marker:focus-visible {
  opacity: 1;
}

.bp_tier_marker_stale {
  inset-inline-end: 1.35rem;
  color: var(--bp-color-accent-warning, var(--bp-color-text-faint));
  font-weight: 700;
  opacity: 0.85;
}

/* The marker is absolutely positioned inside the block it describes. */
.bp_card2_proof_source,
.bp_external_decl_rendered,
.bp_external_decl_signature,
.bp_trust_code {
  position: relative;
}
