(function () {
  const graphGlobal = typeof globalThis !== "undefined" ? globalThis : window;
  const graphRuntimeCore = graphGlobal.VersoBlueprintGraphRuntimeCore || {};
  const debounce = graphRuntimeCore.debounce;
  const normalizeGraphOptions = graphRuntimeCore.normalizeGraphOptions;
  const graphPackAttr = graphRuntimeCore.graphPackAttr;
  const graphOptionsKey = graphRuntimeCore.graphOptionsKey;
  const readPreviewBehaviorDefaults = graphRuntimeCore.readPreviewBehaviorDefaults;
  const layoutGraphCanvas = graphRuntimeCore.layoutGraphCanvas;
  const load = graphRuntimeCore.load;
  const graphNodeLabel = graphRuntimeCore.graphNodeLabel;
  const graphNodeId = graphRuntimeCore.graphNodeId;
  const ensureGraphBlockState = graphRuntimeCore.ensureGraphBlockState;
  const rememberGraphLayoutMeasurements = graphRuntimeCore.rememberGraphLayoutMeasurements;
  const resizeRenderedGraphToCanvas = graphRuntimeCore.resizeRenderedGraphToCanvas;
  const resetGraphvizForVariant = graphRuntimeCore.resetGraphvizForVariant;
  const makeGroupPanelPositioner = graphRuntimeCore.makeGroupPanelPositioner;
