...
"""
    ...
    def test_run_accepts_ready_docs_only_exemptions(self) -> None:
        # Exemptions are now verified against the source PR's changed files (cb3d84a): a docs-only
        # change is legitimately exempt, but only with a source PR number and a token to look it up.
        with tempfile.TemporaryDirectory() as tmp:
            event_path = Path(tmp) / "event.json"
            write_pull_request_event(
                event_path,
    ...
    def test_run_rejects_source_change_exemptions(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_pull_request_event(
                event_path,
                draft=False,
                body=f"{backport_line(REQUIRED_BACKPORT_RELEASE, 'exempt: no reported release-line regression')}\n",
            )
