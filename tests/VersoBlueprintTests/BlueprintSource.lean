...
#eval
  show IO Bool from do
    ...
    let hasSource (document page : String) (entry : Informal.PreviewManifest.Entry) : Bool :=
      entry.sources.any fun sourceRef =>
        sourceRef.document == document &&
          sourceRef.spans.size == 1 &&
          sourceRef.spans[0]!.page == page
    let sourced? (document page : String) (entry? : Option Informal.PreviewManifest.Entry) : Bool :=
      entry?.any fun entry => hasSource document page entry
    pure <|
      sourceDocument?.map (·.pdf) == some (some "source/paper.pdf") &&
      notesDocument?.map (·.kind) == some .text &&
      entriesWithSource.any (fun entry => entry.label == sourcedLabel) &&
      entriesWithSource.any (fun entry => entry.label == secondSourcedLabel) &&
      ...
      codeEntry?.any (fun entry =>
        entry.targetKind == .leanDecl &&
          ...
          hasSource "paper" "12" entry &&
          hasSource "notes" "A" entry) &&
      sourced? "paper" "12" entry? &&
      sourced? "notes" "A" secondEntry?
