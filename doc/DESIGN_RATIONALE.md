...
| Relation panel browser activation, loading/error replacement, and cache lookup | `Informal/Block/relation-panel.js` | relation panels emitted by normal, grafted, Slides, and custom generated nodes | single JS owner for behavior; it reuses the Lean-rendered loading body and only owns runtime error diagnostics |
...
| Browser template preview panel behavior | `Commands/preview-runtime.js` `bindTemplatePreviewRoots`, `bindPreviewTriggers`, and panel helpers | summary and code-summary feature scripts | single JS behavior helper; feature scripts only pass selectors/defaults |
...
| Browser inline-preview panel behavior, child panel, footer, and nested hover behavior | `Commands/inline-preview.js` configured with `Commands/preview-runtime.js` `bindPreviewTriggers` and panel helpers | inline Lean links, bibliography links, single relation chips, nested previews | feature-owned header-label/footer/child-panel state; trigger lifetime and panel mechanics use shared runtime helpers |
| Browser graph preview and group-hover behavior | `Commands/graph.js` configured with runtime trigger and panel helpers | graph command output | feature-owned graph state; preview trigger lifetime and panel mechanics use shared runtime helpers |
...
- **The browser render API has two tiers.**
  ...
  scripts also share helper methods for runtime panel creation, panel
