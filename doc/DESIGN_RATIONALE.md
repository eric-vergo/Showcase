...
| Browser preview panel behavior | `Commands/preview-runtime.js` `bindTemplatePreviewRoots`, `bindPreviewTriggers`, `bindPanelRepositioner`, and panel helpers | summary, code-summary, inline-preview, and graph feature scripts | single JS behavior helper; feature scripts only pass selectors/defaults and feature-specific positioning callbacks |
...
| Browser graph preview, group-hover, popover, and reposition behavior | `Commands/graph.js` configured with runtime trigger, popover, reposition, and panel helpers | graph command output | feature-owned graph state; preview trigger lifetime, popover binding, and resize/scroll binding use shared runtime helpers |
