...
| A custom page that starts from a label and can render external TeX or Markdown fallback source | [`renderNode(element, request)`](#browser-runtime-api) | This renders the generated node shell for native previews and external-markup fallbacks; external renderers own only the content slot. |
